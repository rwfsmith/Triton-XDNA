...
module attributes {transform.with_named_sequence} {
  transform.named_sequence @__transform_main(%arg1: !transform.any_op {transform.readonly}) {
    ...
    // Cast cmpf inputs from f32 to bf16 -- AIE only supports bf16 vector cmp.
    // Only cast inputs (indices 0,1), NOT the result (which must stay i1).
    %vector_cmps = transform.structured.match ops{["arith.cmpf"]} in %vectorized_herd : (!transform.any_op) -> !transform.any_op
    %cmp_cast = transform.air.vector_type_cast %vector_cmps {target_element_type = bf16, input_indices = [0, 1]} : (!transform.any_op) -> !transform.any_op

    // Cast select value operands from f32 to bf16 -- AIE only supports bf16 select.
    // Select has 3 operands: condition (i1), true_val, false_val.
    // Cast inputs 1,2 (the values), not input 0 (the condition).
    %vector_selects = transform.structured.match ops{["arith.select"]} in %vectorized_herd : (!transform.any_op) -> !transform.any_op
    %sel_cast = transform.air.vector_type_cast %vector_selects {target_element_type = bf16, input_indices = [1, 2], output_indices = [0]} : (!transform.any_op) -> !transform.any_op
